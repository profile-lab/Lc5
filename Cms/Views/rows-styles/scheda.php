...
<form class="form" method="POST" enctype="multipart/form-data" action="">
    ...
    */ ?>
    ...
    <div class="checkboxs_cnt_flex">
        <?php if (isset($rows_attributes) && is_iterable($rows_attributes)) { ?>
            <?php foreach( $rows_attributes as $row_attr ) { ?>
                <?= view('Lc5\Cms\Views\form-cmp/checkbox', ['item' => ['label' => 'Hide '. $row_attr['label'], 'name' => $row_attr['name'], 'value' => $row_attr['active'], 'width' => 'col-md-3']]) ?>
            <?php } ?>
        <?php } ?>
    </div>
